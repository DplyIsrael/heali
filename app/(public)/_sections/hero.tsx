...
export function Hero() {
  return (
    ...
    <section className="relative h-[880px] w-full overflow-hidden rounded-[20px]">
      ...
      {/*
        Frosted glass overlay on right 702px — Figma technique:
        A gradient div with filter:blur(40px) + backdrop-blur(1px), rotated 180deg.
        The blur on the gradient itself creates the soft frosted diffusion effect.
      */}
      {/* Frosted overlay on right — main gradient tint */}
      <div
        className="absolute right-0 top-0 h-full w-[702px] pointer-events-none"
        style={{
          background:
      ...
      <div className="absolute inset-0 flex items-center">
        <div className="relative mx-auto w-full max-w-[1440px] h-full">
          <div className="absolute right-[60px] top-1/2 -translate-y-1/2 flex flex-col gap-10 items-end w-[631px]">

            {/* Headline + subtext */}
            <div className="flex flex-col gap-[18px] items-end w-[599px]">
              ...
              <p className="text-[20px] font-light text-[#f3f3f3] text-right w-full">
                בHeali תמצאו את המטפלים שמרגישים לכם נכון – כי ריפוי מתחיל בחיבור אישי.
              </p>
            </div>

            {/* Search — glassmorphism container */}
            <div className="w-full bg-[rgba(255,255,255,0.31)] border border-[rgba(255,255,255,0.33)] rounded-[16px] shadow-[0px_21px_44.6px_-10px_rgba(0,0,0,0.08)] p-4 flex items-center">
              <div className="w-[599px] h-[62px] bg-white border border-[#a7d6b4] rounded-[16px] flex items-center justify-between overflow-hidden px-[10px]">
            ...
            {/* Popular categories */}
            <div className="flex flex-col gap-[18px] items-end w-full">
              <p className="text-[16px] text-[#f3f3f3] text-right w-full">
                קטגוריות פופולריות
              </p>
              <div className="flex gap-[14px] items-center w-full">
                {CATEGORIES.map((cat) => (
                  <Link
                    ...
                    className="h-[40px] px-4 flex items-center gap-[10px] backdrop-blur-[3px] bg-[rgba(0,0,0,0.35)] border border-[#f3f3f3] rounded-bl-[16px] rounded-tl-[16px] rounded-tr-[16px] whitespace-nowrap transition-colors hover:bg-[rgba(0,0,0,0.5)]"
                  >
                    {/* Text first → right in RTL */}
                    <span className="text-[18px] text-[#f3f3f3]">{cat}</span>
                    {/* Arrow icon second → left in RTL */}
